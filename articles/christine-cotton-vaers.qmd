...
The following analysis applies to VAERS reports for Pfizer-BioNTech (non-bivalent) from 2020 through the most recent quarterly update. Signals require EB05 > 2.0 with at least 2 of 4 methods independently flagging the drug–event pair. EB05 is the conservative lower bound of the Bayesian posterior credible interval — at this threshold the disproportionality is unlikely to be a chance fluctuation.
...
menstrual |>
  ...
  tab_header(
    ...
    subtitle = "VAERS · Bayesian disproportionality · EB05 ≥ 1.5, ≥2 methods"
  ) |>
  fmt_number(columns = EB05, decimals = 2) |>
  cols_align(align = "left", columns = Event)
```
...
Cotton's argument was never "the vaccine should not have been developed." It was narrower and more damaging: **the safety data from the trial was not reliable enough to inform the benefit-risk decision that regulators made.**

The signals above do not confirm her trial-level analysis — they can't, because VAERS and a randomized trial measure different things. What they show is narrower and still meaningful: the categories of adverse event she singled out — cardiac, reproductive, thrombotic — are not empty. They surface as real disproportionality signals in independent post-market data, found by independent methods, at conservative thresholds. Some of that is genuine biological signal; some is the reporting dynamics described above. Disentangling the two is the work that proper active surveillance — not VAERS, and not a 7-day trial window — is supposed to do.

That is Cotton's enduring point, and it survives her: the instruments we used to clear these products were not built to see the events that mattered most. She was looking in the right direction.

---

## A Note on Methodology

All signals in this analysis are drawn from the Global Patient Safety VAERS signal database, computed using four independent disproportionality methods: GPS (Gamma-Poisson Shrinker / EBGM), PRR, ROR, and BCPNN/IC. The EB05 measure is the 5th-percentile lower bound of the two-component Gamma mixture posterior — a conservative credible bound, not the geometric mean. Signals require EB05 > 2.0 with ≥2 methods flagging independently. The full interactive database is available at [faers.mobi](https://faers.mobi).

VAERS data is passive and subject to under-reporting. Signals indicate statistical disproportionality, not established causation. The appropriate response to a VAERS signal is investigation, not dismissal.
